...
class App extends React.Component {
  ...
  handleSelectRobbie(robbie) {
    let isSelected = robbie.isSelected;
    let robbies = this.state.robbies;
    robbies.map(robo => (robo = { ...robo, isSelected: !robo.isSelected }));
    this.setState({
      selectedRobbie: { ...robbie, isSelected: !isSelected },
      robbies: robbies
    });
  }
        ...
        <div className="row bg-light app-container" id="container">
          ...
          <div className="col-xs-12 col-md-8 central-panel-container ">
            ...
            <div className="row central-panel-robo-container ">
              <FilterMenu />
              <RobotsView
                robbies={this.state.robbies}
                onSelectRobbie={this.handleSelectRobbie.bind(this)}
              />
            </div>
          </div>
          <div className="col-xs-6 col-md-4 robo-details-container">
            <RoboDetails robbie={this.state.selectedRobbie} />
          </div>
        </div>
      </div>
    );
